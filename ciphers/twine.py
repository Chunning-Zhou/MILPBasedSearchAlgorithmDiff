...
class class_twine:
	def __init__(self):
		...
		self.num_of_sbox = 1
		self.nibble = 8
		self.sbox = [[12, 0, 15, 10, 2, 11, 9, 5, 8, 3, 13, 7, 1, 14, 6, 4]]
		self.pbox_left_DC = [0,1,2,3,8,9,10,11,24,25,26,27,16,17,18,19,\
		12,13,14,15,4,5,6,7,20,21,22,23,28,29,30,31]
		self.pbox_right_DC = [8,9,10,11,0,1,2,3,12,13,14,15,4,5,6,7,\
		24,25,26,27,16,17,18,19,28,29,30,31,20,21,22,23]
		self.pbox_left_LBAS = [0,2,6,4,3,1,5,7]
		self.pbox_right_LBAS = [2,0,3,1,6,4,7,5]
		self.IN_pbox_left_LBAS = [0,5,1,4,3,6,2,7]
		self.IN_pbox_right_LBAS = [1,3,0,2,5,7,4,6]
		...
		self.max_round_of_solve_LBAS = 16
		self.max_round_of_solve_AS = 0
		self.max_round_of_solve_DC = 0
	...
	def fix_diff(self,model_goal,model_filename,diff_type,r,diff_pattern):
		pattern = copy.deepcopy(diff_pattern)
		temp = copy.deepcopy(pattern)
		if diff_type == "diff_pattern_front":
			r1 = r-1
			for i in range(8):
				temp[self.IN_pbox_left_LBAS[i]] = pattern[i] 
			pattern = copy.deepcopy(temp)
			temp_1 = copy.deepcopy(pattern)
			for i in range(8):
				temp_1[self.IN_pbox_right_LBAS[i]] = pattern[i] 
			pattern = copy.deepcopy(temp_1)
